        ...
        <div className={currentMode === 'Dark' ? 'dark h-full' : 'h-full'}>
            <BrowserRouter>
                <div className="flex relative dark:bg-main-dark-bg">
                    ...
                    <div style={{ width: '100vw', maxWidth: '24rem', zIndex: '99999' }} className="w-96 max-w-96 fixed sidebar bg-white dark:bg-secondary-dark-bg">
                        <Sidebar />
                    </div>
                        ...
                        <DialogComponent style={{ maxWidth: '1500px' }} cssClass={`${currentMode === 'Dark' ? 'card-dialog-dark' : 'card-dialog-light'}`} id="cardModal" isModal showCloseIcon={true} animationSettings={cardPopupAnimationSettings} visible={cardPopupOpen && selectedCard} width={'90%'} header={`Learn more about ${selectedCard?.name}`} close={() => setCardPopupOpen(false)}>
                            <div className="flex-col lg:flex-row flex justify-between lg:justify-center w-full h-auto min-h-96 max-h-auto mr-auto ml-auto">
                                <div className="w-full mt-5 order-2 lg:order-1">
                                    <div style={{ borderColor: `${currentMode === 'Dark' ? '#A21537' : 'rgba(0,0,0,0.3)'}` }} className="flex justify-between text-sm font-medium border-b-[2px] pb-2 mt-4">
                                    ...
                                    <div style={{ overflowY: 'scroll', borderColor: `${currentMode === 'Dark' ? '#A21537' : 'rgba(0,0,0,0.3)'}` }} className="flex justify-between text-sm font-medium border-b-[2px] pb-2 mt-4 max-h-52">
                                        <h2 style={{ fontSize: '16px' }}>Text</h2>
                                        <p className="text-lg max-w-md pr-3 pt-10 pb-10 lg:pt-0 lg:pb-0">{selectedCard?.text}</p>
                                    </div>
                                </div>
                                <div className="w-[80%] max-w-[450px] lg:w-[650px] block mx-auto order-1 lg:ml-5 lg:order-2">
